...
class DeadReckoningNode(Node):
    def __init__(self):
        ...
        # Змінні стану для Dead Reckoning (позиція та кут)
        self.x = 0.0
        self.y = 0.0
        self.theta = 0.0
        self.last_time = None

        # Змінні для збереження реальної позиції (Ground Truth) з Gazebo
        self.gt_x = 0.0
        self.gt_y = 0.0
    ...
    def cmd_callback(self, msg: TwistStamped):
        # Визначаємо поточний час у секундах
        current_time = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
        
        # Якщо це перше повідомлення, просто запам'ятовуємо час і виходимо
        if self.last_time is None:
            self.last_time = current_time
            return

        # Обчислюємо dt (різницю часу з минулого кроку)
        dt = current_time - self.last_time
        self.last_time = current_time

        # Отримуємо команди швидкості
        v = msg.twist.linear.x
        w = msg.twist.angular.z

        # Кінематична модель (інтегрування)
        self.x += v * math.cos(self.theta) * dt
        self.y += v * math.sin(self.theta) * dt
        self.theta += w * dt

        # Розраховуємо та виводимо похибку між математикою і реальністю
        error = math.sqrt((self.x - self.gt_x)**2 + (self.y - self.gt_y)**2)
        self.get_logger().info(f"Drift Error: {error:.3f} m")

        # Створюємо точку (Pose) для нашого шляху (Path)
        pose = PoseStamped()
        ...
        # Перетворюємо кут (Yaw) у кватерніон для Z та W
        ...
        # Обмежуємо кількість точок, щоб не перевантажити пам'ять
    ...
    def gt_callback(self, msg: Odometry):
        # Зберігаємо реальні координати з Gazebo для порівняння (Ground Truth)
